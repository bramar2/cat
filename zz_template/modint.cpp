...
public:
	...
	modint& operator++() { val = sanitize(val + 1); return *this; }
	modint& operator--() { val = sanitize(val + MOD - 1); return *this; }
	modint operator++(int) { auto old = *this; operator++(); return old; }
	modint operator--(int) { auto old = *this; operator--(); return old; }

	friend modint operator+(const modint lhs, const modint rhs) { return modint(lhs.val + rhs.val); }
	friend modint operator-(const modint lhs, const modint rhs) { return modint(lhs.val - rhs.val); }
	friend modint operator*(const modint lhs, const modint rhs) { return modint(lhs.val * rhs.val); }
	friend modint operator/(const modint lhs, const modint rhs) { if(rhs == 0) throw; return modint(lhs.val * inv(rhs.val)); }
	...
	modint& operator+=(const modint rhs) { this->val = sanitize(this->val + rhs.val); return *this; }
	modint& operator-=(const modint rhs) { this->val = sanitize(this->val - rhs.val); return *this; }
	modint& operator*=(const modint rhs) { this->val = sanitize(this->val * rhs.val); return *this; }
	modint& operator/=(const modint rhs) { if(rhs == 0) throw; this->val = sanitize(this->val * inv(rhs.val)); return *this; }

	long long value() {
		return val;
	}
	static modint undefined() {
		modint m; m.val = LLONG_MAX; return m;
	}
};